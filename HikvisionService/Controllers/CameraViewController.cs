...
public class CameraViewController : Controller
{
    ...
    private readonly ILogger<CameraViewController> _logger;

    public CameraViewController(HikvisionDbContext context, IHikvisionService hikvisionService, ILogger<CameraViewController> logger)
    {
        _context = context;
    ...
    // POST: /CameraView/Edit/5
    [HttpPost]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> Edit(long id, CameraViewModel model)
    {
        if (id != model.Id)
        {
    ...
    // GET: /CameraView/TestConnection/5

...
public class StoreViewController : Controller
{
    ...
    private readonly ILogger<StoreViewController> _logger;

    public StoreViewController(HikvisionDbContext context, ILogger<StoreViewController> logger)
    {
        _context = context;
        _logger = logger;
    }
    ...
    [HttpPost]
    [ValidateAntiForgeryToken]
    public async Task<IActionResult> Edit(long id, StoreViewModel model)
    {
        if (id != model.Id)
        {
